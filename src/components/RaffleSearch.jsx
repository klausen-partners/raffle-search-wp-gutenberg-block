...
export default function RaffleSearch({ searchUid }) {
	...
	let hiddenTypes = window.raffleSettings?.hiddenTypes || '';
	if (typeof hiddenTypes === 'string') {
		hiddenTypes = hiddenTypes
			.split(',')
			.map((t) => t.trim().toLowerCase())
			.filter(Boolean);
	}
	const typesMode = window.raffleSettings?.typesMode || 'exclude';
		...
		return Object.keys(counts)
			...
			.filter(
				({ value }) =>
					...
					(typesMode === 'exclude'
						? !hiddenTypes.includes(value)
						: hiddenTypes.includes(value)),
			);
	}, [results, hiddenTypes, typesMode]);
